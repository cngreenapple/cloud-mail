...
const dbInit = {
	...
	async v3_0DB(c) {
		...
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN black_subject TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
			console.warn(`Skip column: ${e.message}`);
		}

		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN black_content TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
			console.warn(`Skip column: ${e.message}`);
		}

		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN black_from TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
			console.warn(`Skip column: ${e.message}`);
		}
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'This project is for learning and communication purposes only. It should not be used for illegal activities.\n' +
			'<br>\n' +
			'Please comply with local laws and regulations. The author assumes no legal responsibility.'
