...
const emailService = {
	...
	async send(c, params, userId) {
		...
		let {
			...
			name, //Sender name
			sendType, //Send type
			emailId, //Email ID (for reply emails)
		...
		//Check send count limit if not admin
		...
		if (c.env.admin !== userRow.email) {
			//User doesn't have domain permission
		...
		//Use Cloudflare Email Service first for external, otherwise Resend
		if (!allInternal) {
		...
		//Replace cid image tags with public URLs
		...
		//Increment user send count if role has limit
		...
		//Save directly if all recipients are internal
		if (allInternal) {
	...
	async HandleOnSiteEmail(c, receiveEmail, sendEmailData, attList) {
		...
		for (const email of receiveEmail) {
			...
			//Convert sender email to received mail
			...
			//Set email to recipient's info if they exist
			if (accountRow) {
				...
				//Bounce if recipient lacks domain permission or has email interception
		...
		let message = ''
		//If bounced, update sender email status
		if (bouncedEmail) {
